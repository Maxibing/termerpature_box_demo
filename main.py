...
class Main(Temp_box_gui, ModBus):
    ...
    def display_temperature(self):
        ...
        try:
            self.le_current_temp.setText(str(result["temperature_measurement"]/100))
            self.le_final_temp.setText(str(result["temperature_set_final"]/100))
            ...
            if self.cyclic_temp_run_flag:
                if result["temperature_measurement"]/100 <= self.cyclic_low_temp and result["temperature_set_final"]/100 != self.cyclic_high_temp:
                    self.set_temperature(self.cyclic_high_temp)
                    self.record_log("Set temperature: %d" % self.cyclic_high_temp)
                if result["temperature_measurement"]/100 >= self.cyclic_high_temp and result["temperature_set_final"]/100 != self.cyclic_low_temp:
                    self.set_temperature(self.cyclic_low_temp)
                    self.record_log("Set temperature: %d" % self.cyclic_low_temp)
    ...
    def open_serial(self):
        ...
        try:
            ...
            self.listen_thread = threading.Thread(target=self.listen_modbus, daemon=True)
    ...
    def open_close_log_record(self):
        if self.chk_box_log.isChecked():
            ...
            self._save_log = threading.Thread(target=self.save_log_to_file, daemon=True)
    ...
    def create_log_file(self):
        self.log_file = "Temperature_box_%s.log" % str(time.strftime('%Y_%m_%d_%H_%M_%S',time.localtime(time.time())))
    ...
    def start_cyclic_run(self):
        ...
        try:
            ...
            if temp_low < LOW_TEMPERATURE or temp_high > HIGH_TEMPERATURE:
                self.record_log("Set temperature failed: %d ~ %d" % (temp_low, temp_high))
                return
            else:
                self.cmd_lock.acquire()
