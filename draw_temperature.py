...
def get_temperatures_from_log(file):
    ...
    while _tmp:
        ...
        if "temperature_measurement" in _tmp:
            _tmp = _tmp.split(" ")
            _time = datetime.datetime.strptime(("%s %s" % (_tmp[0], _tmp[1][:8])), "%Y-%m-%d %H:%M:%S")
...
def draw_temp(times, temps):
    ...
    # y轴单位精度
    info = "Start time: %s \nEnd time: %s \nHighest temperature: %.2f °C\nLowest temperature: %.2f °C" % (times[0], times[-1], np.max(temps), np.min(temps))    
    # 显示11个数字
    # plt.xticks(range(1, len(times), int(len(times)/10)), times[::int(len(times)/10)]) 
    # 字体倾斜70°
    # plt.xticks(rotation=70) 
    ax1.plot(times, temps, color="red", linewidth=1.0, linestyle="-", label=info)   
    # label位置
    ...
    ax2.plot(times, temps_step, "r.", color="blue", label="The diff of 2 temperature")
    # label位置
    plt.legend(loc="upper left")    
    # 设置网格样式
    plt.grid(linestyle='-.')    
